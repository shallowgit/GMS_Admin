...
public class AdminService implements UserDetailsService {
    ...
    * */
    ...
    public PageBean<Gms_User> queryUserByKey(int currentPage, int currentCount, String query_key) {
        PageBean pageBean = new PageBean();
        pageBean.setCurrentPage(currentPage);
        pageBean.setCurrentCount(currentCount);
        int totalCount = adminMapper.getUserTotalCountByKeyQuery(query_key);
        pageBean.setTotalCount(totalCount);
        int totalPage = (int) Math.ceil(1.0*totalCount/currentCount);
        pageBean.setTotalPage(totalPage);
        int index = (currentPage-1)*currentCount;
        List<Gms_User> user = adminMapper.queryUserByKey(index,currentCount,query_key);
        pageBean.setList(user);
        return pageBean;
    }

    public void updateUser(Gms_User gms_user) {
        adminMapper.updateUser(gms_user);
    }

    public void addUser(Gms_User gms_user) {
        adminMapper.addUser(gms_user);
    }

/*    public void delUser(Gms_User gms_user) {
        adminMapper.delUser(gms_user);
    }*/
    ...
    public List<Object> queryVenueType(){
        return adminMapper.queryVenueType();
    }

    public PageBean<Gms_Equipment> queryEquipmentByKey(int currentPage, int currentCount, String query_key) {
        PageBean pageBean = new PageBean();
        pageBean.setCurrentPage(currentPage);
        pageBean.setCurrentCount(currentCount);
        int totalCount = adminMapper.getEquipmentTotalCountByKeyQuery(query_key);
        pageBean.setTotalCount(totalCount);
        int totalPage = (int) Math.ceil(1.0*totalCount/currentCount);
        pageBean.setTotalPage(totalPage);
        int index = (currentPage-1)*currentCount;
        List<Gms_Equipment> equipment = adminMapper.queryEquipmentByKey(index,currentCount,query_key);
        pageBean.setList(equipment);
        return pageBean;
    }

    public void addEquipment(Gms_Equipment gms_equipment) {
        adminMapper.addEquipment(gms_equipment);
    }

    public void updateEquipment(Gms_Equipment gms_equipment) {
        adminMapper.updateEquipment(gms_equipment);
    }

    public void delEquipment(Gms_Equipment gms_equipment) {
        adminMapper.delEquipment(gms_equipment);
    }
    ...
    public int getLeaseTotalCountByKeyQuery(String query_key) {
        return adminMapper.getLeaseTotalCountByKeyQuery(query_key);
    }

    public List<Gms_Equipment_Lease> queryLeaseByKey(int index, int currentCount, String query_key, String sortStatus) {
        return adminMapper.queryLeaseByKey(index,currentCount,query_key,sortStatus);
    }
    ...
    public List<Gms_Equipment> queryAllEquipment() {
        return adminMapper.queryEquipmentByKey(0,99999999,"");
    }
    ...
    public void addVdState(Gms_Vdstate vdstate) {
        adminMapper.addVdState(vdstate);
    }

    public void delVdState(String venue_id) {
        adminMapper.delVdState(venue_id);
    }

    public List<Gms_User> queryAllUser() {
        return adminMapper.queryUserByKey(0,99999999,"");
    }
}
